    ...
    <div style={{
      ...
      {total > 1 && (
        <button onClick={() => setIdx(i => (i - 1 + total) % total)} style={{
          ...
          border: "none", borderRadius: "50%", color: "#fff", fontSize: 28,
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <img src={current.src} alt={current.label || ""}
          style={{ maxWidth: "88vw", maxHeight: "80vh", objectFit: "contain", borderRadius: 6, boxShadow: "0 8px 60px rgba(0,0,0,0.8)", display: "block" }} />
        {current.label && (
          <div style={{ marginTop: 14, color: "#fff", fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", opacity: 0.55 }}>{current.label}</div>
        )}
        {total > 1 && (
          <div style={{ marginTop: 8, color: "#fff", fontFamily: "var(--font-mono)", fontSize: 10, opacity: 0.3, letterSpacing: "0.2em" }}>{idx + 1} / {total}</div>
        )}
      </div>
  ...
  useEffect(() => {
    if (!dots.current) {
      dots.current = Array.from({ length: dark ? 80 : 40 }, () => ({
        ...
        s: Math.random() * 0.7 + 0.3, d: Math.random() * 4 + 2, delay: Math.random() * 4,
      }));
    }
  }, [dark]);
  if (!dots.current) {
  ...
  return (
    <div className="hero-bg">
      {dots.current.map((d, i) => (
        <div key={i} className={dark ? "star-bg" : "blossom-bg"} style={{
          left: d.x + "%", top: d.y + "%",
          width: (dark ? 3 : 16) * d.s + "px", height: (dark ? 3 : 16) * d.s + "px",
          background: dark ? ["#FFFFFF","#B6FF3D","#FFC1A0","#B5A7FF","#94EAD8"][i % 5] : ["#FFD7DD","#FFB3C0","#FFE6EC"][i % 3],
          boxShadow: dark ? `0 0 ${6*d.s}px currentColor` : "none",
          animation: `twinkle ${d.d}s ease-in-out ${d.delay}s infinite`,
          opacity: dark ? 0.8 : 0.6,
        }} />
      ))}
      <style>{`@keyframes twinkle { 0%,100% { transform: scale(1); opacity: 0.4; } 50% { transform: scale(1.4); opacity: 1; } }`}</style>
    </div>
  );
}
...
function App() {
  ...
  // Lightbox state lives HERE at App level — outside modal overflow
  const [lightbox, setLightbox] = useState(null); // { images, index }

  const { enabled: soundOn, toggle: toggleSound, playNote } = useSoundEngine(
    tweaks.soundEnabled, (v) => setTweak("soundEnabled", v)
  );
    ...
    const onKey = (e) => {
      if (e.key.length === 1) {
        ...
        if (buf.includes("sakura") || buf.includes("bethany")) {
          setEaster(true); setTimeout(() => setEaster(false), 4000); buf = "";
        }
      }
    };
    window.addEventListener("keydown", onKey);
  ...
  return (
    <>
        ...
        <FoldSection id="about" title={<><em>About</em> Me</>} count="Hello!">
          <div className="about-grid">
            <div className="about-portrait">
              <img src="assets/about-me_Portfolio.png" alt="Bethany Fung portrait"
        ...
        <FoldSection id="contact" title={<><em>Let's</em> Talk</>} count="Open to work">
          <div className="contact-block">
            <div className="contact-big">
              <a href="mailto:bethanyfnt@gmail.com" data-hover>bethanyfnt<br/>@gmail.com</a>
            </div>
            <div className="contact-list">
              <a href="https://www.linkedin.com/in/bethany-fung-34259a26b/" target="_blank" rel="noopener" data-hover><span>LinkedIn</span><span>↗</span></a>
              <a href="https://www.instagram.com/bybethany.design/" target="_blank" rel="noopener" data-hover><span>Instagram · @bybethany.design</span><span>↗</span></a>
              <a href="mailto:bethanyfnt@gmail.com" data-hover><span>Email</span><span>↗</span></a>
            </div>
          </div>
        </FoldSection>
      ...
      {/* Lightbox renders at App level — no overflow clipping! */}
      {lightbox && (
        <Lightbox
      ...
      <div className={"sound-pill " + (soundOn ? "" : "muted")} onClick={toggleSound} data-hover style={{ cursor: "none" }}>
        <span className="dot"></span>
        <span>{soundOn ? "Sound On · Do Re Mi" : "Sound Off"}</span>
      </div>

      <div className={"easter " + (easter ? "show" : "")}>
        <div className="easter-card">✿ thanks for finding me ✿<small>— Bethany</small></div>
      </div>

      <TweaksPanel title="Tweaks">
        <TweakSection label="Sound" />
        <TweakToggle label="Piano notes (Do Re Mi)" value={tweaks.soundEnabled} onChange={(v) => setTweak("soundEnabled", v)} />
        <TweakSection label="Cursor garden" />
        <TweakToggle label="Show sakura trail" value={tweaks.showSakuraTrail} onChange={(v) => setTweak("showSakuraTrail", v)} />
        <TweakRadio label="Trail density" value={tweaks.trailDensity} options={["sparse", "regular", "lush"]} onChange={(v) => setTweak("trailDensity", v)} />
        <TweakSection label="Theme" />
        <TweakRadio label="Mode" value={tweaks.themeOverride} options={["auto", "light", "dark"]} onChange={(v) => setTweak("themeOverride", v)} />
      </TweaksPanel>
    </>
  );
}
