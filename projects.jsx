...
function ProjectPlaceholder({ project }) {
  // Big number + project title — feels editorial, not slop
  return (
    <div className="ph" style={{ "--ph-color": project.color, color: project.accent }}>
      ...
      style={{ position: "relative", inset: 0, width: "100%", height: "100%" }}>
        ...
        fontSize="220" fontStyle="italic"
        fill={project.accent} opacity="0.9">
          {project.no}
        </text>
...
function ProjectCard({ project, onOpen, theme }) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      inner.style.setProperty("--ty", (t * -18).toFixed(1) + "px");
  ...
  return (
            ...
            <div className="modal-hero">
              <div>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                      Deliverables
                      <strong>{project.deliverables.join(" · ")}</strong>
                    </div>
                }
                </div>
              </div>
          ...
          <div style={{ marginBottom: 40 }}>
              ...
              boxShadow: "var(--shadow-md)"
                ...
                src={(() => {
                  ...
                  const m = project.video.match(/(?:v=|youtu\.be\/)([\w-]+)/);
                  const id = m ? m[1] : "";
                ...
                <a href={project.video} target="_blank" rel="noopener" data-hover
            style={{
            ...
            project.images.gallery.map((g, i) =>
                        ...
                        {g.rotate
                          ? <img src={g.src} alt={g.label || ""}
                              style={{
                                ...
                                height: "auto",
                                maxWidth: "none",
                                /* When rotating 90°, the rendered width = original height. */
                                /* Set the unrotated height to the container's width so it fills horizontally. */
                                transform: `translate(-50%, -50%) rotate(${g.rotate}deg)`,
                                transformOrigin: "center center",
                                /* Make the image's intrinsic height fill the container's width once rotated */
                                /* Hack: scale via CSS — use container width via parent inline-block */
                              }}
                              onLoad={(e) => {
                                ...
                                if (!iw || !ih) return;
                                // After rotation 90°, the displayed width = original height,
                                // displayed height = original width.
                                // We want displayed width = cw (full bleed horizontally).
                                // So we need original height = cw → set img.height = cw.
                                // Then displayed height = original width × (cw / ih).
                                const scale = cw / ih;
                                img.style.width  = (iw * scale) + "px"; // unused dimension after rotate (becomes height)
                                img.style.height = ih * scale + "px";   // becomes width after rotate
                                // Adjust container height to fit the rotated image
                                container.style.aspectRatio = "auto";
                                container.style.height = (iw * scale) + "px";
                              }}
                            />
                          : <img src={g.src} alt={g.label || ""}
    ...
    style={{ display: "block", width: "100%", height: "auto", objectFit: "cover", cursor: "zoom-in" }} />
            ...
            <div key={i} className="gph" style={{ gridColumn: `span ${g.span}` }}>
                      <div className="ph"
              style={{ "--ph-color": project.color, color: project.accent, minHeight: g.h }}>
