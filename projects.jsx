    ...
    <div className="ph" style={{ "--ph-color": project.color, color: project.accent }}>
      <svg viewBox="0 0 400 500" preserveAspectRatio="xMidYMid slice"
        ...
        <text x="50%" y="52%" textAnchor="middle" fontFamily="Instrument Serif, serif"
          fontSize="220" fontStyle="italic" fill={project.accent} opacity="0.9">
          {project.no}
        </text>
        <text x="50%" y="92%" textAnchor="middle" fontFamily="JetBrains Mono, monospace"
          fontSize="14" letterSpacing="2" fill={project.accent} opacity="0.85">
...
function ProjectCard({ project, onOpen, theme }) {
  ...
  useEffect(function() {
    ...
    function onScroll() {
      ...
      var t = Math.max(-1, Math.min(1, center / (window.innerHeight / 2)));
  ...
  return (
    <div ref={ref} className={"project-card " + project.span} onClick={function() { onOpen(project); }}>
      <div className={"thumb " + project.aspect}
        ...
        <div ref={innerRef} className="thumb-inner">
          {project.images && project.images.hero
            ? <img src={project.images.hero} alt={project.title} style={{ width: "100%", height: "100%", objectFit: "contain" }} />
...
function ProjectModal({ project, onClose, onLightbox }) {
  var galleryImages = project && project.images && project.images.gallery
  ...
  if (!project) return <div className="modal-root"></div>;

  return (
    <div className="modal-root open">
      <button className="modal-close" onClick={onClose} data-hover>Close</button>
      <div className="modal-curtain">
        <div className="modal-content">
              ...
              <div className="meta">
                ...
                {project.deliverables && (
                  <div style={{ gridColumn: "1 / -1" }}>
                    Deliverables<strong>{project.deliverables.join(" · ")}</strong>
                  </div>
                )}
              </div>
            </div>
            <div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--ink-mute)", marginBottom: 12 }}>
                {project.subtitle}
              </div>
              <h1>{project.title}</h1>
              {project.awards && (
                <div style={{ marginTop: 24, display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {project.awards.map(function(a) {
                    return (
                      <span key={a} style={{ padding: "8px 14px", border: "1px solid var(--accent)", color: "var(--accent)", borderRadius: 999, fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase" }}>
                        ★ {a}
                      </span>
                    );
                  })}
          ...
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--ink-mute)", marginBottom: 24 }}>
            Selected Visuals
          </div>

          {project.video && (
            <div style={{ marginBottom: 40 }}>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--ink-mute)", marginBottom: 14 }}>
                Brand video
              </div>
              <div style={{ position: "relative", paddingTop: "56.25%", background: project.color, borderRadius: "var(--radius)", overflow: "hidden", boxShadow: "var(--shadow-md)" }}>
                <iframe
                  src={(function() {
                    var m = project.video.match(/(?:v=|youtu\.be\/)([\w-]+)/);
                    var id = m ? m[1] : "";
                    return "https://www.youtube.com/embed/" + id + "?rel=0&modestbranding=1";
                  })()}
                  title={project.title + " brand video"}
                  frameBorder="0"
              ...
              <a href={project.video} target="_blank" rel="noopener" data-hover style={{ display: "inline-block", marginTop: 12, fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--ink-mute)" }}>
                Watch on YouTube ↗
              </a>
            </div>
          )}
          ...
          <div className="modal-gallery">
            {galleryImages.length > 0
              ? galleryImages.map(function(g, i) {
                  return (
                    <div key={i} className="gph" style={{ gridColumn: "span " + (g.span || 6) }}>
                      <div
                        onClick={function() { onLightbox(galleryImages, i); }}
                        style={{ background: project.color, borderRadius: "var(--radius)", overflow: "hidden", position: "relative", cursor: "zoom-in" }}
                      >
                        <img
                          src={g.src}
                          alt={g.label || ""}
                          style={{ display: "block", width: "100%", height: "auto", objectFit: "cover", pointerEvents: "none" }}
                        />
                        {g.label && (
                          <span style={{ position: "absolute", left: 14, bottom: 14, padding: "6px 12px", background: "rgba(0,0,0,0.65)", color: "#fff", fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.16em", textTransform: "uppercase", borderRadius: 999, zIndex: 2, pointerEvents: "none" }}>
                            {g.label}
                          </span>
                        )}
                      </div>
              ...
              : [
                  { span: 12, h: 460, label: "HERO IMAGE" },
                  { span: 6, h: 360, label: "DETAIL 01" },
                  { span: 6, h: 360, label: "DETAIL 02" },
                  { span: 4, h: 320, label: "MARK" },
                  { span: 8, h: 320, label: "APPLICATION" },
                  { span: 12, h: 480, label: "FULL SPREAD" }
                ].map(function(g, i) {
                  return (
          ...
          <div style={{ marginTop: 100, borderTop: "1px solid var(--line)", paddingTop: 32, display: "flex", justifyContent: "space-between", fontFamily: "var(--font-mono)", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.18em", color: "var(--ink-mute)" }}>
            ...
            <button onClick={onClose} data-hover style={{ color: "var(--accent)", textTransform: "uppercase" }}>
              ← Back to all work
            </button>
          </div>
